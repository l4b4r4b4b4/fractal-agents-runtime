...
class AgentSyncData(BaseModel):
    ...
    config (`config.configurable`) for the `graphs.react_agent.agent.graph()` factory.
...
class AssistantStorageProtocol(Protocol):
    ...
    Implementations are expected to match `server.storage.get_storage()`
...
def _build_assistant_configurable(agent: AgentSyncData) -> dict[str, Any]:
    """Build `config.configurable` for `graphs.react_agent.agent.graph()`.
