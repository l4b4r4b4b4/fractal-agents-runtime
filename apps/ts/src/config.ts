 ...
 * ## Version Management
 *
 * The runtime version is read from `package.json` at import time — that file
 * is the **single source of truth**. Never hardcode version strings elsewhere;
 * always import `VERSION` from this module.
 */

// Import tracing module early so LANGCHAIN_TRACING_V2 is set before
// any LangChain code is loaded.
import { isLangfuseEnabled } from "./infra/tracing";

// Single source of truth: version is read from package.json.
// Bun supports JSON imports natively — no build step needed.
import packageJson from "../package.json";

/** Current runtime version. Derived from `package.json` — do NOT hardcode. */
...
export interface AppConfig {
  ...
  /**
   * Agent sync scope. Controls which agents are synced from Supabase at startup.
   *
   * - `"none"` (default) — no startup sync (lazy only)
   * - `"all"` — sync all active agents
   * - `"org:<uuid>"` — sync agents for a single organization
   * - `"org:<uuid>,org:<uuid>"` — sync multiple organizations
   */
  agentSyncScope: string | undefined;

  /** Langfuse secret key. Enables tracing when set alongside `langfusePublicKey`. */
  langfuseSecretKey: string | undefined;

  /** Langfuse public key. Enables tracing when set alongside `langfuseSecretKey`. */
...
export function loadConfig(): AppConfig {
  return {
    ...
    agentSyncScope: process.env.AGENT_SYNC_SCOPE || undefined,
