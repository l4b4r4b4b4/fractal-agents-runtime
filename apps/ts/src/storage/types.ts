 ...
 * in-memory implementation lives in `./memory.ts`. A future Postgres
 * implementation will satisfy the same interfaces.
 *
 * Design decisions (v0.0.1):
 *   - No `owner_id` parameter — authentication is deferred to Goal 25.
 *   - No `StoreStorage` or `CronStore` — not in v0.0.1 scope.
