 ...
 *
 * Each prompt is registered via `registerDefaultPrompt()` at import time
 * so that `seedDefaultPrompts()` can auto-create them in Langfuse on
 * first startup.
 *
 ...
 *
 * **IMPORTANT:** These prompt names MUST match the Python runtime exactly
 * so that both engines fetch the same prompts from Langfuse when configured.
 *
 * Reference: apps/python/src/graphs/research_agent/prompts.py
 */

import { registerDefaultPrompt } from "../../infra/prompts";

// ---------------------------------------------------------------------------
// Phase 1 — Broad search
// ---------------------------------------------------------------------------

export const ANALYZER_PHASE1_PROMPT = `\
...
export const WORKER_PHASE1_PROMPT = `\
...
export const AGGREGATOR_PHASE1_PROMPT = `\
...
export const ANALYZER_PHASE2_PROMPT = `\
...
export const WORKER_PHASE2_PROMPT = `\
...
export const AGGREGATOR_PHASE2_PROMPT = `\
...
/**
 * Prompt name → default text mapping.
 *
 * Names MUST match the Python runtime exactly (same Langfuse prompts).
 * See: apps/python/src/graphs/research_agent/prompts.py → _PROMPT_REGISTRY
 */
const PROMPT_REGISTRY: ReadonlyArray<readonly [string, string]> = [
...
// Register defaults at import time (side-effect).
for (const [promptName, defaultText] of PROMPT_REGISTRY) {
  registerDefaultPrompt(promptName, defaultText);
}

/**
 * All Langfuse prompt names registered by this module.
 *
 * Useful for test assertions — verify these match the Python list exactly.
 */
export const PROMPT_NAMES: readonly string[] = PROMPT_REGISTRY.map(
  ([name]) => name,
);
